...
type
  ...
  private
    ...
    procedure SimplifiedInsertObject(const AObject: TValue);
    procedure SimplifiedUpdateObject(const AObject: TValue);
  ...
  public
    ...
    procedure SimplifiedInsert<T: class>(const AObject: T);
    procedure SimplifiedUpdate<T: class>(const AObject: T);
...
procedure TQueryBuilder.SimplifiedInsert<T>(const AObject: T);
begin
  ExecuteInTrasaction(
    procedure
    begin
      SimplifiedInsertObject(AObject);
    end);
end;

procedure TQueryBuilder.SimplifiedInsertObject(const AObject: TValue);
begin
  var OutputFieldList: TArray<TField> := nil;
  var OutputFieldNameList: TArray<String> := nil;
  var Table := TMapper.Default.FindTable(AObject.TypeInfo);
...
procedure TQueryBuilder.SimplifiedUpdate<T>(const AObject: T);
begin
  ExecuteInTrasaction(
    procedure
    begin
      SimplifiedUpdateObject(AObject);
    end);
end;

procedure TQueryBuilder.SimplifiedUpdateObject(const AObject: TValue);
begin
  var SQL := EmptyStr;
  var Table := TMapper.Default.FindTable(AObject.TypeInfo);
