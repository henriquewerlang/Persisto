...
begin
  ...
  begin
    ...
    FCache.Add(TClassLoader.GenerateInternalCacheKey(CacheKey), TObjectHelper.Copy(Result));
  end;
...
begin
  var CurrentObjectFieldStringValue, ForeignFieldStringValue: String;
  var CurrentObjectFieldValue, ForeignFieldValue: TValue;
  var ForeignObject := AObject.AsObject;
  var InternalObject: TObject := nil;
  var SQL := EmptyStr;
  var Table := TMapper.Default.FindTable(AObject.TypeInfo);

  var CacheKey := Table.GetCacheKey(ForeignObject);

  if FCache.Get(CacheKey, Result) and FCache.Get(TClassLoader.GenerateInternalCacheKey(CacheKey), InternalObject) then
  begin
