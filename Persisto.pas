...
type
  ...
  private
    ...
    function GetIdention(Count: Integer): String;
    function GetJoin: TQueryBuilderJoin;
    function GetLineBreak: String;
    function GetMinimalIdention(Count: Integer): String;
  ...
  private
    ...
    function BuildStateObjectKey(const Table: TTable; const &Object: TObject): String;
    ...
    procedure UpdateTable(const Table: TTable; const &Object: TObject);
  public
...
begin
  ...
  begin
    ...
    begin
      ...
      AddTableForeignKey(Table, Table.FPrimaryKey, Table.BaseTable, True);
...
constructor TClassLoader.Create(const Manager: TManager);
begin
  inherited Create;
...
//          FieldValue := Field.Table.PrimaryKey.GetValue(CurrentObject);
        ...
        begin
          ...
          Manipulator.Loader := CreateLazyFactory(FManager, Field, FieldValue)
        end
        else
...
begin
  ...
  begin
    ...
    Result := Where.Where(Condition).BuildCommand;
  end;
end;
...
function TQueryBuilder.GetIdention(Count: Integer): String;
begin
  if not (boBeautifyQuery in Options) then
    Count := 0;

  Result := GetMinimalIdention(Count)
end;
...
function TQueryBuilder.GetMinimalIdention(Count: Integer): String;
begin
  if not (boBeautifyQuery in Options) then
    Count := Min(Count, 1);
...
begin
  if Assigned(FSelect) then
    if Assigned(FFrom) then
      Result := GetIdention(3) + 'select ' + FSelect.GetSQL + GetMinimalIdention(5) + 'from ' + FFrom.BuildCommand
    else
...
//function TQueryBuilder.GetWhere: TQueryBuilderWhere;
//begin
//  if not Assigned(FWrite) then
//    FWrite := TQueryBuilderWhere.Create(Self);
//
//  Result := FWrite;
//end;
...
function TQueryBuilderFrom.BuildCommand: String;
begin
  Result := BuildJoinSQL;
...
constructor TQueryBuilderSelect.Create(const QueryBuilder: TQueryBuilder);
begin
  inherited Create;
...
function TQueryBuilderWhere.BuildCommand: String;
begin
  Result := FQueryBuilder.GetIdention(4) + 'where ' + BuildFilter + FQueryBuilder.GetLineBreak;
...
//  FLoader := TClassLoader.Create(QueryBuilder.GetManager);
end;
...
constructor TQueryBuilderAllFields.Create(const QueryBuilder: TQueryBuilder);
begin
  inherited Create;
...
constructor TQueryBuilderOrderBy.Create(const QueryBuilder: TQueryBuilder);
begin
  inherited Create;
...
begin
  FStateObjects.Add(BuildStateObjectKey(Table, StateObject.&Object), StateObject);
end;

function TManager.BuildStateObjectKey(const Table: TTable; const &Object: TObject): String;
begin
  if Table.HasPrimaryKey then
    Result := Format('%s.%s', [Table.DatabaseName, VarToStr(Table.PrimaryKey.GetValue(&Object).AsVariant)])
  else
    Result := EmptyStr;
end;
...
var
  ...
  DelayInsert: TArray<TField>;

  FieldValue: TValue;

  procedure InsertForeignTable(const Field: TField);
  begin
    try
      if not InsertTable(Field.ForeignKey.ParentTable, FieldValue.AsObject) then
    ...
    try
      ...
      for Field in Table.Fields do
        if not Field.AutoGenerated then
          if Field.IsForeignKey and not Field.Required then
            DelayInsert := DelayInsert + [Field]
          else
          begin
            if Field.HasValue(&Object, FieldValue) and Field.IsForeignKey then
            begin
              InsertForeignTable(Field);

              FieldValue := Field.ForeignKey.ParentTable.PrimaryKey.GetValue(FieldValue.AsObject);
            end;
...
begin
  ...
  begin
    ...
    if DelayInsert <> nil then
    begin
      for var Field in DelayInsert do
    ...
    try
      ...
      begin
        ...
        begin
          ...
          UpdateTable(Field.ForeignKey.ParentTable, ForeignObject);

          FieldValue := Field.ForeignKey.ParentTable.PrimaryKey.GetValue(ForeignObject);
        end;

        if StateObject.OldValue[Field].AsVariant <> FieldValue.AsVariant then
          CreateParam(Field, FieldValue);
      end;

      if Params.Count > 0 then
      begin
        CreateParam(Table.PrimaryKey, Table.PrimaryKey.GetValue(&Object));
...
procedure TManager.UpdateTable(const Table: TTable; const &Object: TObject);
begin
