...
{$M-}
  ...
  EAssociationLinkError = class(Exception)
  public
    constructor Create(ParentTable, AssociatedTable: TTable);
  end;
  ...
  public
    ...
    property Associations: TArray<TAssociation> read FAssociations;
  ...
  TAssociation = class
  private
    FAssociatedField: TField;
    FAssociatedTable: TTable;
    FField: TField;
  public
    constructor Create(const Field: TField; const AssociatedTable: TTable; const AssociatedField: TField);

    property AssociatedField: TField read FAssociatedField;
    property AssociatedTable: TTable read FAssociatedTable;
    property Field: TField read FField;
  end;
  ...
  private
    ...
    procedure AddTableAssociation(const Table: TTable; const Field: TField);
  ...
  private
    FAlias: String;
    FAssociationField: TAssociation;
    FAssociationTables: TList<TQueryBuilderTable>;
    ...
    FLazyAssociationFields: TArray<TField>;
    FPrimaryKeyField: TQueryBuilderTableField;
    FTable: TTable;
    FLazyTables: TList<TQueryBuilderTable>;
  public
    constructor Create(const AssociationField: TAssociation); overload;
    ...
    property AssociationField: TAssociation read FAssociationField write FAssociationField;
    property AssociationTables: TList<TQueryBuilderTable> read FAssociationTables write FAssociationTables;
    ...
    property LazyAssociationFields: TArray<TField> read FLazyAssociationFields write FLazyAssociationFields;
...
{ EAssociationLinkError }

constructor EAssociationLinkError.Create(ParentTable, AssociatedTable: TTable);
begin
  inherited CreateFmt('The link between [%s] and [%s] can''t be maded. Check if it exists, as the same name of the parent table or has the attribute defining the name of the link!',
    [ParentTable.ClassTypeInfo.Name, AssociatedTable.ClassTypeInfo.Name]);
end;
...
procedure TMapper.AddTableAssociation(const Table: TTable; const Field: TField);
...
begin
  ...
  begin
    var LinkName := GetAssociationLinkName;

    for var AssociatedField in AssociatedTable.Fields do
      if AssociatedField.Name = LinkName then
        Field.FAssociation := TAssociation.Create(Field, AssociatedTable, AssociatedField);

    if Assigned(Field.Association) then
      Table.FAssociations := Table.FAssociations + [Field.Association]
    else
      raise EAssociationLinkError.Create(Table, AssociatedTable);
  end
  else
...
begin
  ...
  begin
    ...
    if Field.IsAssociation then
      AddTableAssociation(Table, Field)
...
function TMapper.SortFieldFunction(const Left, Right: TField): Integer;
  ...
  begin
    if Field.InPrimaryKey then
      ...
      Result := 3
    else if Field.IsAssociation then
      Result := 4
    else
      Result := 2;
  end;
...
begin
  ...
  for var Association in Associations do
    Association.Free;
...
constructor TAssociation.Create(const Field: TField; const AssociatedTable: TTable; const AssociatedField: TField);
begin
  inherited Create;

  FAssociatedField := AssociatedField;
  FAssociatedTable := AssociatedTable;
  FField := Field;
end;
...
begin
  ...
  begin
    ...
    if FLazyField.IsAssociation then
      FilterField := FLazyField.Association.AssociatedField
    ...
    if FLazyField.IsAssociation or FLazyField.IsForeignKey then
...
var
  LoadedObjects: TDictionary<String, TObject>;
  ...
  function BuildStateObjectKeyForAssociationObject(const QueryTable: TQueryBuilderTable): String;
  begin
    Result := '#.' + BuildStateObjectKey(QueryTable);
  end;

  function BuildStateObjectKeyForAssociationProperty(const QueryTable: TQueryBuilderTable): String;
  begin
    Result := '*.' + BuildStateObjectKey(QueryTable);
  end;
  ...
  function CheckAssociationPropertyLoaded(const QueryTable: TQueryBuilderTable): Boolean;
  begin
    Result := not LoadedObjects.TryAdd(BuildStateObjectKeyForAssociationProperty(QueryTable), nil);
  end;

  function CheckAssociationObjectLoaded(const QueryTable: TQueryBuilderTable): Boolean;
  begin
    Result := not LoadedObjects.TryAdd(BuildStateObjectKeyForAssociationObject(QueryTable), nil);
  end;

  function CheckObjectLoaded(const QueryTable: TQueryBuilderTable; const &Object: TObject): Boolean;
  begin
    Result := not LoadedObjects.TryAdd(BuildStateObjectKeyForObject(QueryTable), &Object);
  end;
  ...
  begin
    ...
    for Field in QueryTable.LazyAssociationFields do
      Field.LazyValue[&Object] := CreateLazyFactory(Field, QueryTable.PrimaryKeyField.Field.Value[&Object]);

    if not QueryTable.AssociationTables.IsEmpty and not CheckAssociationPropertyLoaded(QueryTable) then
      for AssociationTable in QueryTable.AssociationTables do
        AssociationTable.AssociationField.Field.Value[&Object] := nil;

    for AssociationTable in QueryTable.AssociationTables do
      if not AssociationTable.PrimaryKeyField.DataSetField.IsNull then
      begin
        AssociationObject := CreateObject(AssociationTable);

        if not CheckAssociationObjectLoaded(AssociationTable) then
        begin
        ...
        LoadFieldValues(AssociationTable, AssociationObject);
      end;
  end;

begin
  var Cursor := FQueryBuilder.OpenCursor;
  LoadedObjects := TDictionary<String, TObject>.Create;
  ...
  begin
    ...
    if not CheckObjectLoaded(FQueryBuilder.FQueryTable, &Object) then
...
var
  ...
  begin
    ...
    for var AssociationTable in QueryTable.AssociationTables do
      LoadDataSetFields(AssociationTable);
  end;
...
var
  ...
  begin
    ...
    for Field in QueryTable.Table.Fields do
      if Field.IsInheritedLink then
        ...
        if Field.IsLazy then
          QueryTable.LazyAssociationFields := QueryTable.LazyAssociationFields + [Field]
        else
          QueryTable.AssociationTables.Add(TQueryBuilderTable.Create(Field.Association))
    ...
    for AssociationTable in QueryTable.AssociationTables do
      try
        LoadFieldList(AssociationTable, AssociationTable.AssociationField.AssociatedField);
      except
        on E: ERecursionSelectionError do
          raise ERecursionSelectionError.Create(Format('%s.%s->%s', [QueryTable.Table.Name, AssociationTable.AssociationField.Field.Name, E.RecursionTree]));
      end;
  end;
  ...
  procedure MakeAssociationJoin(const QueryTable, AssociationTable: TQueryBuilderTable);
  begin
    MakeJoin(QueryTable, QueryTable.PrimaryKeyField.Field, AssociationTable, AssociationTable.AssociationField.AssociatedField);
  end;
  ...
  begin
    ...
    for var AssociationTable in QueryTable.AssociationTables do
    begin
      MakeAssociationJoin(QueryTable, AssociationTable);

      BuildJoin(AssociationTable);
    end;
  end;
  ...
  function FindQueryField(const FieldNameToFind: TQueryBuilderFieldSearch; var CurrentTable: TQueryBuilderTable): TField;
    ...
    begin
      ...
      begin
        ...
        for var FindTable in QueryTable.AssociationTables do
          if FindTable.AssociationField.Field.Name = FieldName then
            Exit(FindTable);
  ...
  begin
    ...
    try
      ...
      for Field in Table.Fields do
        if not Field.AutoGenerated and not Field.IsAssociation and Field.HasValue(&Object, FieldValue) then
        begin
...
begin
  ...
  FAssociationTables := TObjectList<TQueryBuilderTable>.Create;
  FTable := Table;
end;
...
constructor TQueryBuilderTable.Create(const AssociationField: TAssociation);
begin
  Create(AssociationField.AssociatedTable);

  FAssociationField := AssociationField;
end;
...
var
  ...
  procedure LoadAutoGeneratedAttribute;
  const
    ATTRIBUTE_NAME: array[TAutoGeneratedType] of String = ('AutoGeneratedNoDefined', 'CurrentDate', 'CurrentTime', 'CurrentDateTime', 'NewUniqueIdentifier', 'Sequence(''%s'')', 'FixedValue(''%s'')');

  var
    AutoGeneratedDefinition: String;
    ...
    begin
      var SequenceParts := Format(AutoGeneratedDefinition, [SEQUENCE_SEPARATOR]).Split([SEQUENCE_SEPARATOR]);
      Result := ConstraintValue;

      for var SequencePart in SequenceParts do
        Result := Result.Replace(SequencePart, EmptyStr);
    end;
    ...
    begin
      ConstraintValue := Field.DefaultConstraint.Value;
      Result := EmptyStr;
      ...
      begin
        ...
        case A of
          ...
          begin
            ...
            if Format(AutoGeneratedDefinition, [SequenceName]) = ConstraintValue then
              Exit(Format(AttributeDefinition, [FormatName(SequenceName)]));
          end;
          agtFixedValue: Exit(Format(AttributeDefinition, [GetFixedValue.Replace('''', '''' + '''')]));
          else if AutoGeneratedDefinition = ConstraintValue then
            Exit(AttributeDefinition);
        end;
      end;
    end;

  begin
    AddAttribute(GetAutoGeneratedAttribute);
  end;
