...
type
  ...
  IDatabaseSchemaRepository = interface
    ['{768E8A3E-24B7-40A6-9AD3-03E0F26B8BD5}']
    function GetTables: TArray<TDatabaseTable>;
  end;
  ...
  TDatabaseSchemaRepository<T: TDatabaseTable> = class(TInterfacedObject, IDatabaseSchemaRepository)
  private
    FManager: TManager;
  ...
  private
    ...
    FMapper: TMapper;
    FMetadaManipulator: IDatabaseManipulator;
  ...
  public
    constructor Create(const Connection: IDatabaseConnection; const MetadaManipulator: IDatabaseManipulator);
...
begin
  ...
  FDatabaseManipulator := Manager.FMetadaManipulator;
  FManager := Manager;
...
//        Exit(True);
  ...
  begin
    Result := Assigned(DatabaseField.DefaultConstraint) xor Assigned(Field.DefaultConstraint) or Assigned(DatabaseField.DefaultConstraint)
      and ((DatabaseField.DefaultConstraint.Name <> FDatabaseManipulator.GetDefaultConstraintName(Field))
  ...
  begin
    ExecuteDirect(Format('create table %s (Id varchar(10))', [Table.DatabaseName]));
  end;

  procedure LoadDatabaseTables;
  begin
    for var DatabaseTable in FDatabaseManipulator.CreateSchemaRepository(FManager).GetTables do
...
//
  for Table in FManager.Mapper.Tables do
    if not DatabaseTables.ContainsKey(Table.DatabaseName) then
    begin
      CreateTable;
...
constructor TManager.Create(const Connection: IDatabaseConnection; const MetadaManipulator: IDatabaseManipulator);
begin
  inherited Create;

  FConnection := Connection;
  FMapper := TMapper.Create;
  FMetadaManipulator := MetadaManipulator;
...
var
  ...
  begin
    ...
    try
      ...
      var Cursor := FConnection.PrepareCursor(FMetadaManipulator.MakeInsertStatement(Table, Params), Params);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        FConnection.PrepareCursor(FMetadaManipulator.MakeUpdateStatement(Table, Params), Params).Next;
      end;
    finally
      Params.Free;
    end;
...
{ TDatabaseSchemaRepository<T> }

constructor TDatabaseSchemaRepository<T>.Create(const Manager: TManager);
begin
  inherited Create;
...
function TDatabaseSchemaRepository<T>.GetTables: TArray<TDatabaseTable>;
begin
  Result := TArray<TDatabaseTable>(FManager.Select.All.From<T>.Open.All);
end;
