...
var
  ...
  begin
    ...
    var LazyBuildIn := TLazyBuildInType.Create;
...
procedure TClassLoaderTest.WhenLoadAClassWithALazyFieldWithABuildInTypeMustLoadTheValueAsExpected;
begin
  var LazyValue := FManager.Select.All.From<TLazyBuildInType>.Where(Field('Key') = 'Lazy').Open.One;

  Assert.AreEqual('Value', LazyValue.LazyString.Value)
end;

procedure TClassLoaderTest.WhenLoadAClassWithALazyPropertyAndThanReloadTheClassMustResetTheLazyFieldValue;
begin
  var AObject := CreateObject<TNameLazyField>;
  AObject.Lazy := CreateObject<TNameLazyFieldAnother>;
  AObject.Lazy.Value.Field := 'Value 1';

  FManager.Save([AObject]);

  FManager.Select.All.From<TNameLazyField>.Open.One.Lazy.Value;

  AObject.Lazy.Value.Field := 'Value 2';

  FManager.Save([AObject]);

  var AnotherObject := FManager.Select.All.From<TNameLazyField>.Open.One;

  Assert.AreEqual('Value 2', AnotherObject.Lazy.Value.Field);
end;
