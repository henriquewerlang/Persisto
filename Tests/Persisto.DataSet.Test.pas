...
type
  ...
  public
    ...
    [Test]
    procedure WhenApplyUpdatesOfNotEditingDataSetCantRaiseAnyError;
    [Test]
    procedure WhenDeleteAnRecordAndApplyTheUpdateTheObjectMustBeRemovedFromTheDatabase;
    [Test]
    procedure WhenDeleteARecordMustRemoveThisObjectFromTheList;
    [Test]
    procedure WhenDeleteARecordFromAChildDataSetMustUpdateTheArrayInTheParentObject;
    [Test]
    procedure WhenTheChildrenDataSetGetTheManagerMustUseTheManagerFromTheParentDataSet;
    [Test]
    procedure WhenApplyUpdateInTheChildrenDataSetCantUpdateTheValuesInTheDatabase;
    [Test]
    procedure WhenDeleteAChildrenRecordAndApplyTheUpdateMustRemoveThisValueFromDatabase;
    [Test]
    procedure WhenApplyTheChangesInTheParentDataSetMustApplyTheChangedInTheNestedDataSets;
  end;
...
uses System.Classes, System.Variants, Data.DBConsts, Persisto.Test.Entity, Persisto.Test.Connection;
...
procedure TPersistoDataSetTest.SetupFixture;
begin
...
procedure TPersistoDataSetTest.TearDownFixture;
begin
  FManager.Free;
end;
...
begin
  ...
  Assert.AreEqual(TDataSetField, FDataSet.FieldByName('Children').ClassType);
end;
...
procedure TPersistoDataSetTest.WhenApplyTheChangesInTheParentDataSetMustApplyTheChangedInTheNestedDataSets;
begin
  var AObject := TMyManyValue.Create;
  var Children: TArray<TMyChildLink> := [TMyChildLink.Create];
  var ChildrenField := TDataSetField.Create(FDataSet);
  ChildrenField.FieldName := 'Children';
  FDataSet.Objects := [AObject];
  var NestedDataSet := TPersistoDataSet.Create(FDataSet);

  AObject.Children := Children;

  ChildrenField.SetParentComponent(FDataSet);

  NestedDataSet.DataSetField := ChildrenField;
  ...
  var Child := FManager.Select.All.From<TMyChildLink>.Where(Field('Id') = Children[0].Id).Open.One;

  Assert.IsNil(Child);
end;

procedure TPersistoDataSetTest.WhenApplyTheUpdatesMustPostTheCurrentRecord;
begin
  FDataSet.ObjectClass := TAutoGeneratedClass;
...
procedure TPersistoDataSetTest.WhenApplyTheUpdatesMustUpdateTheValuesInTheDatabase;
begin
  FDataSet.ObjectClass := TAutoGeneratedClass;
  ...
  var AObject := FDataSet.GetCurrentObject<TAutoGeneratedClass>;
  AObject.Value := 'My Value';

  FDataSet.ApplyUpdates;

  Assert.IsNotEmpty(AObject.Id);

  AObject := FManager.Select.All.From<TAutoGeneratedClass>.Where(Field('Id') = AObject.Id).Open.One;

  Assert.AreEqual('My Value', AObject.Value);
end;

procedure TPersistoDataSetTest.WhenApplyUpdateInTheChildrenDataSetCantUpdateTheValuesInTheDatabase;
begin
  var AObject := TMyManyValue.Create;
  var Children: TArray<TMyChildLink> := [TMyChildLink.Create];
  var ChildrenField := TDataSetField.Create(FDataSet);
  ...
  NestedDataSet.ApplyUpdates;

  Assert.IsEmpty(Children[0].Id);
end;

procedure TPersistoDataSetTest.WhenApplyUpdatesOfNotEditingDataSetCantRaiseAnyError;
begin
  FDataSet.ObjectClass := TAutoGeneratedClass;

  FDataSet.Open;

  Assert.WillNotRaise(FDataSet.ApplyUpdates);
end;

procedure TPersistoDataSetTest.WhenTheChildrenDataSetGetTheManagerMustUseTheManagerFromTheParentDataSet;
begin
  var ChildrenField := TDataSetField.Create(FDataSet);
  ChildrenField.FieldName := 'Children';
  var NestedDataSet := TPersistoDataSet.Create(FDataSet);
  ...
  Assert.AreEqual(FDataSet.Manager, NestedDataSet.Manager);
end;
...
begin
  ...
  Assert.AreEqual(ftDataSet, FDataSet.FieldByName('Children').DataType);
end;
...
begin
  var AObject := TMyManyValue.Create;
  AObject.Children := [TMyChildLink.Create, TMyChildLink.Create, TMyChildLink.Create];
  var ChildrenField := TDataSetField.Create(FDataSet);
  ChildrenField.FieldName := 'Children';
  FDataSet.Objects := [AObject];
  var NestedDataSet := TPersistoDataSet.Create(FDataSet);
  ...
  for var Child in AObject.Children do
    Child.Free;
end;
...
procedure TPersistoDataSetTest.WhenDeleteAChildrenRecordAndApplyTheUpdateMustRemoveThisValueFromDatabase;
begin
  var AObject := TMyManyValue.Create;
  var Children: TArray<TMyChildLink> := [TMyChildLink.Create];
  var ChildrenField := TDataSetField.Create(FDataSet);
...
procedure TPersistoDataSetTest.WhenDeleteAnRecordAndApplyTheUpdateTheObjectMustBeRemovedFromTheDatabase;
begin
  FDataSet.ObjectClass := TAutoGeneratedClass;
  ...
  var Id := FDataSet.FieldByName('Id').AsString;

  FDataSet.Delete;

  FDataSet.ApplyUpdates;

  var AObject := FManager.Select.All.From<TAutoGeneratedClass>.Where(Field('Id') = Id).Open.One;

  Assert.IsNil(AObject);
end;

procedure TPersistoDataSetTest.WhenDeleteARecordFromAChildDataSetMustUpdateTheArrayInTheParentObject;
begin
  var AObject := TMyManyValue.Create;
  var Children: TArray<TMyChildLink> := [TMyChildLink.Create, TMyChildLink.Create, TMyChildLink.Create];
  var ChildrenField := TDataSetField.Create(FDataSet);
  ChildrenField.FieldName := 'Children';
  FDataSet.Objects := [AObject];
  var NestedDataSet := TPersistoDataSet.Create(FDataSet);
  ...
  Assert.AreEqual(2, Length(AObject.Children));
end;

procedure TPersistoDataSetTest.WhenDeleteARecordMustRemoveThisObjectFromTheList;
begin
  FDataSet.Objects := [TMyTestClass.Create, TMyTestClass.Create, TMyTestClass.Create];
...
begin
  ...
  Assert.AreEqual(3, Length(AObject.Children));
...
begin
  ...
  Object1.Children := [TMyChildLink.Create, TMyChildLink.Create, TMyChildLink.Create];
  Object3.Children := [TMyChildLink.Create];
  var ChildrenField := TDataSetField.Create(FDataSet);
  ChildrenField.FieldName := 'Children';
  ...
  for var Child in Object1.Children + Object2.Children + Object3.Children do
    Child.Free;
end;
...
begin
  ...
  for var Child in Object1.Children do
    Child.Free;
end;
