...
uses System.Generics.Collections, System.Classes, Data.DB, Test.Insight.Framework, Persisto, Persisto.Mapping;
...
type
  ...
  public
    ...
    [Test]
    procedure WhenUseALazyAssociationFieldInTheFilterCantRaiseAnyError;
    [Test]
    procedure WhenFilterALazyAssociationMustBuildTheQueryWithoutAnyError;
    [Test]
    procedure WhenFilterALazyAssociationMustLoadTheValueAsExpected;
    [Test]
    procedure WhenUseALazyFieldInTheFilterMustLoadTheTableJoinOnlyOnce;
    [Test]
  ...
  public
    constructor Create(const Owner: TComponent; const Connection: IDatabaseConnection); reintroduce;
...
function TManagerTest.CreateManager: TPersistoManager;
begin
...
procedure TManagerTest.PrepareDatabase;
  ...
  begin
    ...
    var AssociationClass := CreateObject<TAssociationClass>;

    AssociationClass.LazyFieldAssociation := CreateObject<TAssociationChildClass>;
    AssociationClass.LazyFieldAssociation.Value.Value := 123;

    FManager.Insert([AssociationClass]);
  end;
...
begin
  ...
  FManager.Connection := FConnection;
...
procedure TManagerTest.WhenFilterALazyAssociationMustBuildTheQueryWithoutAnyError;
begin
  Assert.WillNotRaise(
    procedure
    begin
      FManager.Select.All.From<TAssociationClass>.Where(Field('LazyFieldAssociation.Value') = 123).Open.One;
    end);
end;

procedure TManagerTest.WhenFilterALazyAssociationMustLoadTheValueAsExpected;
begin
  var Value := FManager.Select.All.From<TAssociationClass>.Where(Field('LazyFieldAssociation.Value') = 123).Open.One;

  Assert.IsNotNil(Value);
end;

procedure TManagerTest.WhenFilterALazyAssociationUsingTheSameFieldMoreThanOnceCantRaiseAnyError;
begin
  Assert.WillNotRaise(
    procedure
    begin
      FManager.Select.All.From<TAssociationClass>.Where((Field('LazyFieldAssociation.Value') = 123) and (Field('LazyFieldAssociation.Value') = 123) and (Field('LazyFieldAssociation.Value') = 123)).Open.One;
    end);
end;
...
begin
  ...
  Manager.Select.All.From<TAssociationClass>.Where((Field('LazyFieldAssociation.Value') = 123) and (Field('LazyFieldAssociation.Value') = 123) and (Field('LazyFieldAssociation.Value') = 123)).Open.One;

  Assert.AreEqual('select T1.Id F1,T2.Id F2,T2.Value F3 from AssociationClass T1 left join AssociationChildClass T2 on T2.IdAssociationClass=T1.Id left join AssociationChildClass T3 on T3.IdAssociationClass=T1.Id where T3.Value=:P1 and T3.Value=:P2 and T3.Value=:P3',
    DatabaseConnection.LastCommandExecuted);
end;

procedure TManagerTest.WhenUseALazyAssociationFieldInTheFilterCantRaiseAnyError;
begin
  Assert.WillNotRaise(
    procedure
    begin
      FManager.Select.All.From<TLazyArrayClass>.Where(Field('LazyArray.Id').IsNull).Open.One;
    end);
end;
...
procedure TManagerTest.WhenUseALazyFieldInTheFilterMustLoadTheTableJoinOnlyOnce;
begin
  var DatabaseConnection := TDatabaseConnectionMock.Create(FManager, CreateConnection(FManager));
  var Manager := CreateManager;
  Manager.Connection := DatabaseConnection;
...
constructor TDatabaseConnectionMock.Create(const Owner: TComponent; const Connection: IDatabaseConnection);
begin
