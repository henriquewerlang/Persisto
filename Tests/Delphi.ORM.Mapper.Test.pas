...
type
  ...
  public
    ...
    [Test]
    procedure WhenTheFieldHasntDefaultAttributeTheFunctionHasValueMustReturnTrue;
    [Test]
    procedure IfTheFieldHasDefaultValueAttributeAndTheValueIsTheSameOfTheDefaultMustFalseInTheHasValueFunction;
    [Test]
    ...
    [Test]
    procedure IfTheFieldIsAutoGeneratedMustLoadTheDefaultValueForTheField;
    [Test]
...
procedure TMapperTest.IfTheFieldHasDefaultValueAttributeAndTheValueIsTheSameOfTheDefaultMustFalseInTheHasValueFunction;
begin
  var MyClass := TMyEntityWithDefaultValue.Create;
  MyClass.AEnum := Enum3;
  var Table := FMapper.LoadClass(MyClass.ClassType);
  var Value: TValue;

  Assert.IsFalse(Table.Field['AEnum'].HasValue(MyClass, Value));
...
procedure TMapperTest.IfTheFieldIsAutoGeneratedMustLoadTheDefaultValueForTheField;
begin
  var Table := FMapper.LoadClass(TAutoGeneratedClass);

  Assert.IsFalse(Table.Field['Id'].DefaultValue.IsEmpty);
end;
...
begin
  ...
  Assert.AreEqual('123456', Table.Field['Lazy'].GetAsString(TheValue));
...
begin
  var Table := FMapper.LoadClass(TMyEntityWithDefaultValue);

  Assert.AreEqual(EncodeDate(2021, 08, 02), Table.Field['Date'].DefaultValue.AsType<TDate>);
end;
...
begin
  ...
  Assert.AreEqual(EncodeDateTime(2021, 08, 02, 11, 0, 30, 0), Table.Field['DateTime'].DefaultValue.AsType<TDateTime>);
end;
...
begin
  ...
  Assert.AreEqual<Double>(123.456, Table.Field['Float'].DefaultValue.AsType<Double>);
end;
...
begin
  ...
  Assert.AreEqual(Enum3, Table.Field['AEnum'].DefaultValue.AsType<TMyEnumerator>);
end;
...
begin
  Assert.WillRaise(
    procedure
    begin
      FMapper.LoadClass(TMyEntityWithInvalidDefaultValue);
...
begin
  ...
  Assert.AreEqual(123456, Table.Field['Int'].DefaultValue.AsType<Integer>);
end;
...
begin
  ...
  Assert.AreEqual(EncodeTime(11, 0, 30, 0), Table.Field['Time'].DefaultValue.AsType<TTime>);
end;
...
procedure TMapperTest.WhenTheFieldHasntDefaultAttributeTheFunctionHasValueMustReturnTrue;
begin
  var MyClass := TMyEntityWithDefaultValue.Create;
  var Table := FMapper.LoadClass(MyClass.ClassType);
  var Value: TValue;

  Assert.IsTrue(Table.Field['Id'].HasValue(MyClass, Value));
...
begin
  ...
  Assert.AreEqual('abcde', Table.Field['AField'].DefaultValue.AsString);
end;
