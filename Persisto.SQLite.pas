...
type
  TPersistoManipulatorSQLite = class(TDatabaseManipulator, IDatabaseManipulator)
  private
...
{ TPersistoManipulatorSQLite }

function TPersistoManipulatorSQLite.CreateDatabase(const DatabaseName: String): String;
begin
  Result := EmptyStr;
end;

function TPersistoManipulatorSQLite.CreateSequence(const Sequence: TSequence): String;
begin
  Result := Format('insert into sqlite_sequence (name, seq) values (''%s'', 0)', [Sequence.Name]);
end;

function TPersistoManipulatorSQLite.DropDatabase(const DatabaseName: String): String;
begin
  Result := EmptyStr;
end;

function TPersistoManipulatorSQLite.DropSequence(const Sequence: TDatabaseSequence): String;
begin
  Result := Format('delete from sqlite_sequence where name = ''%s''', [Sequence.Name]);
end;

function TPersistoManipulatorSQLite.ExtractFixedValue(const Expression: String): String;
begin
  Result := EmptyStr;
end;

function TPersistoManipulatorSQLite.GetAutoGeneratedDefinition(const AutoGenerated: TAutoGeneratedType): String;
const
  AUTO_GENERATED_VALUE_MAPPING: array [TAutoGeneratedType] of String = ('', 'date()', 'time()', 'datetime(''now'', ''localtime'')', 'uuid()', '', '');

begin
  Result := AUTO_GENERATED_VALUE_MAPPING[AutoGenerated];
end;

function TPersistoManipulatorSQLite.GetDefaultDatabaseName: String;
begin
  Result := EmptyStr;
end;

function TPersistoManipulatorSQLite.GetDefaultValue(const Field: TField): String;
begin
  case Field.DefaultConstraint.AutoGeneratedType of
    ...
    Result := GetAutoGeneratedDefinition(Field.DefaultConstraint.AutoGeneratedType);
  end;
end;

function TPersistoManipulatorSQLite.GetFieldType(const FieldType: TTypeKind): String;
begin
  case FieldType of
    tkInteger:
      Result := 'integer';
...
function TPersistoManipulatorSQLite.GetMaxNameSize: Integer;
begin
  Result := 100;
end;

function TPersistoManipulatorSQLite.GetSchemaTablesScripts: TArray<String>;
const
...
function TPersistoManipulatorSQLite.IsSQLite: Boolean;
begin
  Result := True;
end;
