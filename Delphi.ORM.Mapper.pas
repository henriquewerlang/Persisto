...
begin
  if not TryFindTable(TypeInfo.Handle, Result) and (TypeInfo.GetAttribute<SingleTableInheritanceAttribute> = nil) then
  begin
...
begin
  ...
  else if Assigned(BaseClass) and (BaseClass.MetaclassType <> TObject) then
  begin
    var BaseTable := LoadClassInTable(BaseClass);
    var Field := TFieldPrimaryKeyReference.Create;

    LoadFieldInfo(Table, BaseTable.PrimaryKey.PropertyInfo, Field);

    Field.FAutoGenerated := False;
    Field.FFieldType := BaseTable.ClassTypeInfo;
    Table.FPrimaryKey := BaseTable.PrimaryKey;

    AddTableForeignKey(Table, Field, BaseTable);
  end;
end;
