...
type
  ...
  public
    procedure SetValue(Instance: TObject; Value: Variant);
...
procedure TField.SetValue(Instance: TObject; Value: Variant);
begin
  ...
  if Value = NULL then
    ...
    FieldValue := TValue.FromOrdinal(TypeInfo.PropertyType.Handle, Value)
  else if TypeInfo.PropertyType.Handle = System.TypeInfo(TGUID) then
    FieldValue := TValue.From(StringToGuid(Value))
  else
    FieldValue := TValue.FromVariant(Value);

  TypeInfo.SetValue(Instance, FieldValue);
end;
