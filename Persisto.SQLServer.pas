...
type
  TPersistoManipulatorSQLServer = class(TDatabaseManipulator, IDatabaseManipulator)
  private
...
{ TPersistoManipulatorSQLServer }

function TPersistoManipulatorSQLServer.CreateDatabase(const DatabaseName: String): String;
begin
  Result := Format('create database %s', [DatabaseName]);
end;

function TPersistoManipulatorSQLServer.DropDatabase(const DatabaseName: String): String;
begin
...
function TPersistoManipulatorSQLServer.ExtractFixedValue(const Expression: String): String;
begin
  Result := Expression.Substring(1, Expression.Length - 2);

  if Result[1] = '(' then
    Result := ExtractFixedValue(Result);
end;

function TPersistoManipulatorSQLServer.GetAutoGeneratedDefinition(const AutoGenerated: TAutoGeneratedType): String;
const
...
function TPersistoManipulatorSQLServer.GetDefaultDatabaseName: String;
begin
  Result := 'master';
end;

function TPersistoManipulatorSQLServer.GetDefaultValue(const Field: TField): String;
begin
...
function TPersistoManipulatorSQLServer.GetFieldType(const FieldType: TTypeKind): String;
begin
  case FieldType of
    tkInteger:
      Result := 'int';
...
function TPersistoManipulatorSQLServer.GetMaxNameSize: Integer;
begin
  Result := 128;
end;

function TPersistoManipulatorSQLServer.GetSchemaTablesScripts: TArray<String>;
const
...
function TPersistoManipulatorSQLServer.MakeInsertStatement(const Table: TTable; const Params: TParams): String;
begin
