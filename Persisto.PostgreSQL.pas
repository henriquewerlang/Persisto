...
type
  TPersistoManipulatorPostgreSQL = class(TDatabaseManipulator, IDatabaseManipulator)
  private
...
{ TPersistoManipulatorPostgreSQL }

function TPersistoManipulatorPostgreSQL.CreateDatabase(const DatabaseName: String): String;
begin
  Result := Format('create database "%s"', [DatabaseName]);
end;

function TPersistoManipulatorPostgreSQL.DropDatabase(const DatabaseName: String): String;
begin
  Result := Format('drop database if exists "%s" with (force)', [DatabaseName]);
end;

function TPersistoManipulatorPostgreSQL.ExtractFixedValue(const Expression: String): String;
begin
  Result := Expression.Split(['::'])[0];
end;

function TPersistoManipulatorPostgreSQL.GetAutoGeneratedDefinition(const AutoGenerated: TAutoGeneratedType): String;
const
  DEFAULT_VALUE: array[TAutoGeneratedType] of String = ('', 'CURRENT_DATE', 'LOCALTIME(0)', 'LOCALTIMESTAMP(0)', 'gen_random_uuid()', 'nextval(''%0:s''::regclass)', '%s');

begin
  Result := DEFAULT_VALUE[AutoGenerated];
end;

function TPersistoManipulatorPostgreSQL.GetDefaultDatabaseName: String;
begin
  Result := 'postgres';
end;

function TPersistoManipulatorPostgreSQL.GetDefaultValue(const Field: TField): String;
begin
  var DefaultConstraint := Field.DefaultConstraint;
  var DefaultValue := GetAutoGeneratedDefinition(DefaultConstraint.AutoGeneratedType);
...
function TPersistoManipulatorPostgreSQL.GetFieldType(const FieldType: TTypeKind): String;
begin
  case FieldType of
...
function TPersistoManipulatorPostgreSQL.GetMaxNameSize: Integer;
begin
  Result := 63;
end;

function TPersistoManipulatorPostgreSQL.GetSchemaTablesScripts: TArray<String>;
const
  TABLE_SQL =
    '''
